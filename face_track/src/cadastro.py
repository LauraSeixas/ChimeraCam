...
if not os.path.exists(f'../imagens_cadastro/{nome}'):
    os.makedirs(f'../imagens_cadastro/{nome}')
...
while True:
    ...
    if count % (intervalo * fps) == 0 and count_cadastro <= 5:
        # Captura a imagem e salva em um arquivo
        cv2.imwrite('../imagens_cadastro/{}/{}.jpg'.format(nome, count_cadastro), frame)
